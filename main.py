...
from audio_output import initialize_tts, speak
...
def main():
    ...
    try:
        for cap, frame in camera_generator:
            ...
            if key == ord(' '):
                speak(tts_engine, "Analyzing image...")
                ...
                try:
                    ...
                    # Speak the result
                    speak(tts_engine, description)
                except Exception as e:
                    error_msg = f"Failed to analyze image: {e}"
                    print(error_msg)
                    speak(tts_engine, "Sorry, I encountered an error while analyzing the image.")
